...
const HeartParticle = ({
  ...
  delay: number;
  left: number;
}) => <div className="absolute text-rose-500 animate-float-up pointer-events-none" style={{
  left: `${left}%`,
...
}) => <div className="absolute pointer-events-none animate-floating-heart opacity-20" style={style}>
    <Heart className="text-amber-300 fill-amber-300/50" style={{
...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isEmail) {
        // Use secure RPC function to get email by phone (bypasses RLS)
  ...
  return <div className="h-screen flex overflow-hidden relative">
      {/* Back Arrow - Top Left */}
      <Link to="/" className="absolute top-4 left-4 z-50 flex items-center justify-center w-10 h-10 rounded-full bg-black/80 hover:bg-black text-white shadow-lg transition-all duration-300 hover:scale-105">
      ...
      {showHearts && <div className="fixed inset-0 z-50 pointer-events-none overflow-hidden">
          {hearts.map(heart => <HeartParticle key={heart.id} delay={heart.delay} left={heart.left} />)}
        </div>}
      ...
      <div className="hidden lg:flex lg:w-[45%] relative bg-gradient-to-br from-rose-900 via-pink-800 to-rose-900 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          {floatingHearts.map(heart => <FloatingHeart key={heart.id} style={heart.style} />)}
        </div>
        ...
        {/* Content */}
        <div className="relative z-10 w-full p-8 z-[3] mx-0 my-[3px] px-[3px] py-[42px] mr-[3px] flex-col flex items-center justify-center gap-[12px] rounded-full opacity-100 shadow-2xl">
          {/* Logo & Brand */}
          <div className="text-center mb-4">
            <div className="flex items-center justify-center gap-2 mb-2">
              <img src={vikshanaLogo} alt="Vikshana Matrimony" className="h-14 w-14 object-contain drop-shadow-2xl" />
            </div>
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <div className="w-16 h-px bg-gradient-to-r from-transparent via-amber-400 to-transparent" />
            <Heart className="h-3 w-3 text-amber-400 fill-amber-400 animate-pulse" style={{
            animationDelay: "0.5s"
          }} />
          </div>
          ...
          <div className="w-full max-w-md mb-4 overflow-hidden">
            <div className="flex gap-4 animate-scroll-left">
              ...
              {[...weddingImages, ...weddingImages].map((img, index) => <div key={index} className="relative h-44 w-52 flex-shrink-0 rounded-xl overflow-hidden shadow-xl border border-amber-400/20 transform transition-transform duration-500 hover:scale-105">
                  <img src={img} alt={`Wedding ${index % 4 + 1}`} className="w-full h-full object-cover" />
      ...
      <div className="w-full lg:w-[55%] flex items-center justify-center bg-gradient-to-br from-slate-50 via-rose-50 to-amber-50 p-4 lg:p-8">
        <div className="w-full max-w-md">
          ...
          <div className="lg:hidden text-center mb-4">
            <div className="flex items-center justify-center gap-2 mb-2">
              <img src={vikshanaLogo} alt="Vikshana Matrimony" className="h-12 w-12 object-contain" />
            </div>
          ...
          boxShadow: "0 20px 40px -12px rgba(0, 0, 0, 0.12)"
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <Input type="text" placeholder="Enter your phone or email" value={identifier} onChange={e => setIdentifier(e.target.value)} className="h-11 pl-4 pr-12 bg-slate-50 border-slate-200 rounded-xl focus:border-rose-400 focus:ring-rose-400/20 text-slate-800 placeholder:text-slate-400 text-sm" />
              ...
              <div>
                ...
                <div className="relative">
                  <Input type={showPassword ? "text" : "password"} placeholder="Enter your password" value={password} onChange={e => setPassword(e.target.value)} className="h-11 pl-4 pr-16 bg-slate-50 border-slate-200 rounded-xl focus:border-rose-400 focus:ring-rose-400/20 text-slate-800 placeholder:text-slate-400 text-sm" />
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="text-slate-400 hover:text-rose-500 transition-colors">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <Checkbox id="stayLoggedIn" checked={stayLoggedIn} onCheckedChange={checked => setStayLoggedIn(checked as boolean)} className="border-slate-300 data-[state=checked]:bg-rose-500 data-[state=checked]:border-rose-500 h-4 w-4" />
                  <label htmlFor="stayLoggedIn" className="text-xs text-slate-600 cursor-pointer">
                    Stay logged in
                  </label>
                </div>
            ...
            <div className="mt-5 text-center space-y-2">
              <p className="text-slate-500 text-sm">
                ...
                <Link to="/#hero" className="text-rose-500 hover:text-rose-600 font-semibold transition-colors">
                  Register Now
                </Link>
              </p>
              <Link to="/" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-black hover:bg-slate-800 text-white text-xs font-medium transition-all duration-300 shadow-md hover:shadow-lg">
                ← Back to Home
              </Link>
            </div>
          </div>
